 ...
 *   Set an API key in Homey settings. If set, every request (except /health)
 *   must include one of:
...
class McpServer extends EventEmitter {
  ...
  constructor({ port = 52199, tools = new Map(), log, getApiKey }) {
    super();
    this.port      = port;
    this.tools     = tools;
  ...
  async _handleRequest(req, res) {
    ...
    // Auth check — all other endpoints require API key if one is set
